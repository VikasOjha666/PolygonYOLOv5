...
def polygon_inter_union_cpu(boxes1, boxes2):
    ...
    for i in range(n):
        polygon1 = shapely.geometry.Polygon(boxes1[i, :].cpu().view(4,2)).convex_hull

        for j in range(m):
            polygon2 = shapely.geometry.Polygon(boxes2[j, :].cpu().view(4,2)).convex_hull
...
def polygon_b_inter_union_cpu(boxes1, boxes2):
    ...
    union = torch.zeros(n,)
    for i in range(n):
        polygon1 = shapely.geometry.Polygon(boxes1[i, :].cpu().detach().view(4,2)).convex_hull
        polygon2 = shapely.geometry.Polygon(boxes2[i, :].cpu().detach().view(4,2)).convex_hull
        if polygon1.intersects(polygon2):
            try:
